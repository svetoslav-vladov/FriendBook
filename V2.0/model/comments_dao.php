...
function getAllCommentsForCurrentPost($post_id) {
    $pdo = $GLOBALS['pdo'];
    $statement = $pdo->prepare("SELECT comments.id AS comment_id , comments.description, comment_date, post_id, owner_id, profile_pic, first_name, last_name, users.gender, display_name
                                FROM comments
